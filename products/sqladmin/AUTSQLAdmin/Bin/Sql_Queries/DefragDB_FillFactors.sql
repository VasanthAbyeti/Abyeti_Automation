...
if exists (select * from master.dbo.sysdatabases where name='DefragDB')
...
begin
	...
	set @sql = 'Alter index IDX_CL_defrag_1_'+cast(@i as nvarchar(5))+' on defrag_part_table_'+cast(@i as nvarchar(5))+ ' Rebuild with (FILLFACTOR = 80);'
	EXEC (@sql)
	...
	set @sql = 'Alter index IDX_NCL_defrag_2_'+cast(@i as nvarchar(5))+' on defrag_part_table_'+cast(@i as nvarchar(5))+ ' Rebuild with (FILLFACTOR = 60);'
	EXEC (@sql)
	...
	set @sql = 'Alter index IDX_NCL_defrag_3_'+cast(@i as nvarchar(5))+' on defrag_part_table_'+cast(@i as nvarchar(5))+ ' Rebuild with (FILLFACTOR = 20);'
	EXEC (@sql)
	...
	set @sql = 'Alter index IDX_NCL_defrag_4_'+cast(@i as nvarchar(5))+' on defrag_part_table_'+cast(@i as nvarchar(5))+ ' Rebuild with (FILLFACTOR = 90);'
	EXEC (@sql)
	...
	set @sql = 'Alter index IDX_NCL_defrag_5_'+cast(@i as nvarchar(5))+' on defrag_part_table_'+cast(@i as nvarchar(5))+ ' Rebuild with (FILLFACTOR = 80);'
	EXEC (@sql)	
...
--View table stats
select * from sys.dm_db_index_physical_stats (db_id(),object_id('defrag_part_table_1'),2, NULL, 'LIMITED')